...
class FlickrDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        """This function returns a tuple that is further passed to collate_fn
        """
        image_file_name, caption = self.lines[index * 5].split(',', 1)  # TODO mismatch
        uri = os.path.join(self.images_root, image_file_name)
        # with open(os.path.join(self.images_root, image_file_name), 'rb') as fp:
        #     image_buffer = fp.read()
        image = Image.open(uri)
        image = np.asarray(image)
        return image, str(caption).lower().rstrip(), uri
...
def collate_fn(data):
    ...
    images, captions, uris = zip(*data)
    return images, captions, uris
...
def input_index_data(num_docs=None, batch_size=8, dataset_type='f30k'):
    ...
    for i, (images, captions, uri) in enumerate(data_loader):
        # docs = DocumentArray.empty(batch_size) # TODO fix empty document
        docs = DocumentArray()
        for idx, (image, caption, uri) in enumerate(zip(images, captions, uri)):
            hashed = hashlib.sha1(image).hexdigest()
            document_img = Document()

            document_img.tensor = image
            document_img.uri = uri
            document_img.modality = 'image'
            document_img.mime_type = 'image/jpeg'
            docs.append( document_img)
            # yield document_img
